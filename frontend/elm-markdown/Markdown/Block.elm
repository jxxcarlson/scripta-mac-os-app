...
@docs Block, CodeBlock, Fence, ListBlock, ListType, Alignment
...
parseTextLine rawLine ast =
    case ast of
        (Paragraph headerRaw _) :: astTail ->
            case parseTableSeparator rawLine of
                Just alignments ->
                    let
                        headerCells =
                            parseTableRow headerRaw
                    in
                    if List.length headerCells == List.length alignments then
                        Table alignments [ headerCells ] :: astTail
...
tableSeparatorRegex : Regex
tableSeparatorRegex =
    Regex.fromString "^\\|?\\s*(:?-+:?)\\s*(\\|\\s*(:?-+:?)\\s*)*\\|?\\s*$"
        |> Maybe.withDefault Regex.never


tableSeparatorCellRegex : Regex
tableSeparatorCellRegex =
    Regex.fromString ":?-+:?"
        |> Maybe.withDefault Regex.never


parseTableSeparator : String -> Maybe (List Alignment)
parseTableSeparator line =
    if Regex.contains tableSeparatorRegex (String.trim line) then
        Regex.find tableSeparatorCellRegex line
            |> List.map (.match >> parseAlignment)
            |> Just
...
parseAlignment sep =
    let
        leftColon =
            String.startsWith ":" sep

        rightColon =
            String.endsWith ":" sep
    in
    if leftColon && rightColon then
        AlignCenter

    else if rightColon then
        AlignRight

    else if leftColon then
        AlignLeft
...
parseTableRow : String -> List (List (Inline i))
parseTableRow line =
    let
        trimmed =
            String.trim line
    ...
    String.split "|" stripped
        |> List.map (\cell -> [ Text (String.trim cell) ])
...
defaultHtml customHtml customInlineHtml block =
    ...
    case block of
        ...
        Paragraph _ inlines ->
            [ p [ style "margin-top" "1em" ] (List.map inlineToHtml inlines) ]
        ...
        List model items ->
            List.map
                ...
                |> (case model.type_ of
                        Ordered startInt ->
                            if startInt == 1 then
                                ol [ style "padding-left" "2em", style "margin-top" "1em", style "margin-bottom" "0" ]
                            else
                                ol [ start startInt, style "padding-left" "2em", style "margin-top" "1em", style "margin-bottom" "0" ]

                        Unordered ->
                            ul [ style "padding-left" "2em", style "margin-top" "1em", style "margin-bottom" "0" ]
                   )
                |> (\a -> (::) a [])
        ...
        Table alignments rows ->
            let
                ...
                renderCell tag alignment cells =
                    tag (alignAttr alignment ++ [ style "padding-right" "1em" ])
                        (List.map inlineToHtml cells)

                renderRow tag rowCells =
                    let
                        paddedAlignments =
                            alignments ++ List.repeat (List.length rowCells - List.length alignments) AlignDefault
                    in
                    tr []
                        (List.map2 (renderCell tag) paddedAlignments rowCells)
            in
            case rows of
                headerRow :: bodyRows ->
                    [ Html.table
                        ...
                        [ thead [ style "background" "#e8e8e8" ] [ renderRow th headerRow ]
                        , tbody [] (List.map (renderRow td) bodyRows)
                        ]
                    ]
...
walkInlinesHelp function block =
    case block of
        ...
        Table alignments rows ->
            List.map (List.map (List.map (Inline.walk function))) rows
                |> Table alignments
...
queryInlinesHelp function block =
    case block of
        ...
        Table _ rows ->
            List.concatMap (List.concatMap (List.concatMap (Inline.query function))) rows
